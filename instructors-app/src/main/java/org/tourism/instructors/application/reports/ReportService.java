...
public class ReportService {
    ...
    public byte[] exportProtocols(Optional<LocalDate> dateFrom, Optional<LocalDate> dateTill)
            throws IOException {
        ...
        try (Workbook workbook = new XSSFWorkbook();
            ...
            List<ProtocolRepository.ReportAssignmentProjection> data =
                    protocolRepository.getAssignmentsForReport(
                            dateFrom.orElse(MIN_DATE), dateTill.orElse(MAX_DATE));
            for (int i = 0; i < data.size(); i++) {
                ...
                expiresCell.setCellValue(
                        item.getAssignmentDate().plusYears(item.getExpiresInYears()));
    ...
    private static void createHeader(Sheet sheet) {
        ...
        sheet.setAutoFilter(
                new org.apache.poi.ss.util.CellRangeAddress(0, 0, 0, columns.length - 1));
    }
