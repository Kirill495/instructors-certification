...
@Mapper(
        ...
        uses = {GradeMapper.class, KindOfTourismMapper.class})
public interface GradeAssignmentMapper {
    ...
    @Mapping(
            target = "validThrough",
            expression =
                    "java(model.getProtocolDate().plusYears(model.getGrade().getExpiresInYears()))")
    @Mapping(
            target = "valid",
            expression =
                    "java(model.getProtocolDate().plusYears(model.getGrade().getExpiresInYears()).isAfter(java.time.LocalDate.now()))")
