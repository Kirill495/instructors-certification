...
public class TouristServiceImpl implements TouristService {
    ...
    public Page<TouristDTO> getAllTourists(Pageable pageable) {
        ...
        Map<Integer, List<ProtocolRepository.GradeAssignmentProjection>> assignments =
                getAssignments(touristsPage.getContent());
                ...
                touristsPage.stream()
                        .map(
                                tourist ->
                                        touristMapper.toDTO(
                                                tourist,
                                                getTouristAssignments(tourist, assignments)))
                        .toList();
    ...
    public List<TouristSummaryDTO> searchLightTourists(String query) {
        ...
        return searchTouristsInnerByFullName(queryParts).stream()
                .map(touristMapper::toLightDTO)
                .toList();
    }
    ...
    public void delete(int touristId) {
        if (!protocolRepository.getAssignments(List.of(touristId)).isEmpty()) {
            throw new TouristCannotBeDeletedException(
                    "У туриста есть присвоения званий. Невозможно удалить.", touristId);
        }
    ...
    public TouristDTO findTouristById(int id) {
        Tourist tourist =
                touristRepository.findById(id).orElseThrow(() -> new TouristNotFoundException(id));
        List<ProtocolRepository.GradeAssignmentProjection> assignments =
                protocolRepository.getAssignments(List.of(id));
    ...
    public Optional<TouristDTO> findTouristByTelegramId(long chatId) {
        return contactInfoRepository
                .findTouristIdByTypeAndValue(ContactInfoType.TELEGRAM, String.valueOf(chatId))
                .map(ContactInfoItem::getTourist)
                .map(touristMapper::toDTO);
    }

    private @NonNull List<ContactInfoItem> getContactInfo(Tourist tourist) {
        return contactInfoRepository.findAllByTouristId(tourist.getId()).stream()
                .sorted(
                        Comparator.comparing(ContactInfoItem::getType)
                                .thenComparing(ContactInfoItem::getValue))
                .toList();
    }

    private static @NonNull List<ProtocolRepository.GradeAssignmentProjection>
            getTouristAssignments(
                    Tourist tourist,
                    Map<Integer, List<ProtocolRepository.GradeAssignmentProjection>> assignments) {
        return assignments.getOrDefault(tourist.getId(), Collections.emptyList()).stream()
                .sorted(
                        Comparator.comparing(
                                ProtocolRepository.GradeAssignmentProjection::getProtocolDate))
                .toList();
    }

    private @NonNull Map<Integer, List<ProtocolRepository.GradeAssignmentProjection>>
            getAssignments(List<Tourist> tourists) {
        return protocolRepository
                .getAssignments(tourists.stream().map(Tourist::getId).toList())
                .stream()
                .collect(
                        Collectors.groupingBy(
                                ProtocolRepository.GradeAssignmentProjection::getTouristId));
    }
