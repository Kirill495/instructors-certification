...
class GradeControllerMockMVCTest {
    ...
    @InjectMocks
    GradeExceptionHandler exceptionHandler;

    MockMvc mockMvc;

    @BeforeEach
    void setUp() {
        mockMvc = MockMvcBuilders.standaloneSetup(gradeController).setControllerAdvice(exceptionHandler).build();
    ...
    void deleteGrade() throws Exception {
        doThrow(new GradeUsedInProtocolsException(grade)).when(catalogService).deleteGrade(anyInt());
        mockMvc.perform(post("/catalog/grades/1/delete")).andExpect(status().isOk())
                .andExpect(view().name("catalog/grades/view"))
                .andExpect(model().attribute("errorMessage", "Звание grade не может быть удалено " +
    ...
    void deleteNonExistingGrade() throws Exception {
        ...
        mockMvc.perform(post("/catalog/grades/1/delete")).andExpect(status().is3xxRedirection())
